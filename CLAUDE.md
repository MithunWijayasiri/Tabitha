...
Every `sessions` mutation — `add`, `addBackup`, `put`, `remove`, `removeAll`, `removeTab` — goes through one FIFO queue (`createSerializer`, `src/core/utils/serialize.ts`); background wraps its alarm and context-menu handlers in a second instance of the same primitive. Queued, never dropped: a second call waits, so `add` returning `undefined` means the save failed, never that it was skipped. `sessions.busy` is true from enqueue until the queue drains. Consequence: a held key would run a command once per auto-repeat, so `Commands.svelte` ignores `ev.repeat`. Add a new mutation to the queue, not beside it; internal callers (`put` → `remove`, `deleteTab` → `put`) use the raw functions, since a queued call from inside a queued slot deadlocks.

The two queues are per-context, so they cannot serialise a background save against a popup save — both key to `lastSaved`. `saveSession` therefore reads its guard **after** the tab read, not with the filter settings, narrowing the overlap to persist + claim. Still open, not closed: closing it needs popup saves routed through background.
