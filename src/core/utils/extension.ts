import type { Icon, Page } from "@/core/types";
...
export function openFullView() {
  return openPage("popup", "?tab=true");
}
...
export function pageUrl(page?: Page, query: string = "") {
  return `${baseUrl}src/${page}/index.html${query}`;
}
...
const defaultFavIcon = (url: string) =>
  isFirefox ? null : `${baseUrl}_favicon/?pageUrl=${url}&size=16`;
